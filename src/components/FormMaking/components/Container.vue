      ...
      <el-container>
        <el-aside style="wdith: 250px;">
          ...
          <div class="components-list">
            <div class="widget-cate" v-if="elementData.length">信息项字段</div>
            <draggable tag="ul" v-if="elementData.length" :list="elementData"
              v-bind="{group:{ name:'people', pull:'clone',put:false},sort:false, ghostClass: 'ghost'}"
              @end="handleMoveEnd"
              @start="handleMoveStart"
              :move="handleMove"
              >
              <li class="form-edit-widget-label" v-for="(item, index) in elementData" :key="index">
                <a>
  ...
  computed: {
    elementData () {
      ...
      this.value.forEach((item) => {
        data.push(this.findTemplateData(item))
      })
      return data
    }
  },
  methods: {
    findTemplateData (value) {
      let basicComponents = this.basicComponents
      let data = {}
      for (let i = 0, len = basicComponents.length; i < basicComponents.length; i++) {
        if (basicComponents[i].type === value.type) {
          data =  Object.assign({}, basicComponents[i])
